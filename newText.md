...
### Abstract
The Turn-Milling is an essencial part of high performance metal-products with high production cost. By modelling the produced contour before production, costly errors can be prevented. This modelling process is the target of this work, featuring two approaches, one being a practical approach and one being analytical.

### Background
In medical science, automotive and aeronautical applications great accuracy and material strength is needed to achieve best results and longevity of tools. For this purpose often steel is used, but with great strength comes difficult processing.

The Turn-Milling is a process offering both accuracy and the capabilities to cut steel and harder metals, thus fitting all mentioned criteria.
In this process, the workpiece, often a hard-metal-block is fixed to a turn-table and spins at around 20,000 RPM. Then a cutting tool or a mill is lowered into the metal, cutting off layers. Over time this creates a rotational contour of the tool in the workpiece.
...
The contour can be controlled by moving the tool. It was sufficient for us to model the movement of the tool along the $z$ and $x$ axis. Here, the $z$-axis represents the depth of the contour and the $x$-axis the position along the length of the workpiece. For simplicity, it wasn't necessary to model the movement of the mill along the $x$-axis while the milling is in process.
...
The cutting-depth is limited by the rotational-axis along the $x$ and the radius $r$ or hight of the cylinder.

## First approach
As the workpiece is spinning at infinite speed, all changes made at a position $x$ will be applied around the cylinder. Due to the cylindrical shape of the workpiece, the 3D-form of the tool should not make a difference, as only it's deepest cuts have to be taken into account.
This would allow to simplify the model further. Representing the workpiece and tool only as a cross-section in a binary matrix with $r$-rows and $x$-columns, where the $1$ represent the workpiece and the $0$ represent empty space, we should be able to substract the tool from the workpiece. The new matrix would represent a cross-section of the new workpiece with a cutout resembling the tool. By rotating the matrix around the $x$-axis, the 3D-form would be computable.

# First Result and the Problem we faced
This is our **first result**, it still seems to be angular but it describes the form of the cross-section very well even though it is simplified.

![show stair stepping graph](stair_stepping.png)

The remainging problem is that our final tool would be a torus and our code still cannot visualize these difficulties perfectly. To get rid of the step shape profile, we implemented a custom smoothing algorithm.
...
```
Importantly on the day two we moved away from the offset based tool creation system, which seemed like a good idea to get started on the day one, but later it turned out to be very impractical. To solve this we rewrote great parts of the tool code. We also removed the previously mentioned smoothing code which became unnecesary with the vectorization. Instead, we needed to implement an interpolation algorithm. First, we wanted to build a custom algorithm, but then decided to use the built-in numpy.interp function.
...
## II. Second approach
After we communicated with a representative from Moduleworks, we got to know that the cutting part of tool will always be a torus and could be need in a very high resolution. As this would resemble the worst case for our current algorithm, we opted for a more analytical approach.
...
def half_donut(R: int, r: int, scale: int, down = True):
    ...
    return (X,Y,Z)
```
The torus is not defined for values smaller than the inner radius $r§. As this would create unrealistic results, a sub-problem was found. How does a cavity inside a tool change the contour? We will reference this problem in the final conclusion, for now we found the following solution:
For our proof of concept we changed the rotating object. Instead of rotating the workpiece around the $x$ axis, we would rotate the tool around the $x$-axis. When the tool is lowered until it's center cuts the $x$-axis, it's spinning around itself, creating a sphere. This is equal to the assumption that all parts of the tool cut at the same depth.
...
#### Back to 2D
As the symmetrie remained the same and we still assume that all parts of the tool cut at the same depth, we used slices which we could then rotate again.
